...
public class WindowsSystemServices implements Closeable {
    ...
    public interface Service {
        ...
        public interface Listener {
            void extendedServiceStatusChanged(Service service);
        }
        ...
        void configure(WindowsSystemServices service);
    ...
    public WindowsSystemServices(WindowsPlatformServiceImpl app) {
        smgr = new W32ServiceManager();
        load();
        this.app = app;
        task = app.context().queue().scheduleWithFixedDelay(new Runnable() {
            @Override
            public void run() {
                load();
            }
    ...
    private final List<Win32Service> services = new ArrayList<>();
    private final List<ServicesListener> listeners = new ArrayList<>();
    private final WindowsPlatformServiceImpl app;
    ...
    public boolean isStartOnBoot(Service service) throws Exception {
        for (String line : app.commands().privileged().output("cmd.exe", "/c", "sc", "qc", service.getNativeName())) {
    ...
    public void stopService(Service service) throws Exception {
        synchronized (smgr) {
            ...
            try {
                W32Service srv = smgr.openService(service.getNativeName(),
